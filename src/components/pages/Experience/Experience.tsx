  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-1 md:gap-10 px-[10dvw] w-full">
        <span className={twMerge(pageHeading, 'mb-6 font-bold text-text-primary/85 leading-[1.1]')}>
        ...
        <p className={twMerge(mainContent, 'mb-16 max-w-3xl text-text-secondary leading-relaxed')}>
          <AnimateString isMobile={isMobile} delayOffset={0.18}>
            My time at RealtyPlus has been a masterclass in building scalable, real-world
            applications. I've navigated the full lifecycle of software development—from initial
            internship tasks to leading frontend architecture in my full-time role.
          </AnimateString>
        </p>
  ...
  return (
    ...
    >
      ...
      <h3 className={twMerge(contentHeading, 'mb-3 font-bold text-text-primary/95')}>
        <AnimateString isMobile={isMobile} delayOffset={index * 0.12}>
          {title}
        </AnimateString>
      </h3>
      <p
        className={twMerge(contentSubHeading, 'max-w-md text-text-primary/65 leading-relaxed mb-6')}
      >
        <AnimateString isMobile={isMobile} delayOffset={index * 0.12}>
          {desc}
        </AnimateString>
